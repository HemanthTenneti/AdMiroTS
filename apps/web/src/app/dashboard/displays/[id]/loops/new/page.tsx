...
import { Loader2, ArrowLeft, Plus, Trash2, ChevronUp, ChevronDown, Image, Video } from "lucide-react";
...
function useCreateDisplayLoop(displayId: string) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const activeAds = adsRes.data.data.data.filter(
        ad => ad.status.toLowerCase() === "active"
      );
      setAds(activeAds);
    } catch (err) {
      const axiosErr = err as AxiosError<{ message?: string }>;
      toast.error(axiosErr.response?.data?.message ?? "Failed to load data");
    } finally {
      setLoading(false);
    }
  }, [displayId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const response = await displayLoopsApi.create({
      ...
      await Promise.all(
        selectedAds.map((ad, idx) =>
          displayLoopsApi.addAdvertisement(response.data.data.id, {
      ...
      const axiosErr = err as AxiosError<{ message?: string }>;
      toast.error(axiosErr.response?.data?.message ?? "Failed to create loop");
    } finally {
      setSubmitting(false);
    }
  };
...
export default function CreateDisplayLoopPage({
  params,
}: {
