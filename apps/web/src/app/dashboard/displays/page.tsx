...
function SortIcon({ col, sortBy, sortOrder }: { col: string; sortBy: SortColumn; sortOrder: SortOrder }) {
  if (sortBy !== col) return <ChevronsUpDown size={13} className="ml-1 text-white/20" />;
      ...
      <main className="min-h-screen bg-[#080410] p-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-3xl font-bold text-white mb-1 tracking-tight">Displays</h1>
              <p className="text-white/40 text-sm">Manage your display devices</p>
            </div>
            <Link
          ...
          <div className="mb-6">
            <div className="relative flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-9 py-2.5 bg-white/5 border border-white/10 text-white placeholder:text-white/30 focus:border-[#7E3AF0] focus:outline-none rounded-lg text-sm"
            ...
            {activeSearchTerm && (
              <p className="mt-2 text-xs text-white/40">
                Found <span className="text-white/70 font-medium">{totalItems}</span> display{totalItems !== 1 ? "s" : ""} for &ldquo;{activeSearchTerm}&rdquo;
              </p>
            )}
          </div>
          ...
          {loading ? (
            <div className="flex items-center justify-center h-64">
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-white/40 text-sm">Loading displays…</p>
              </div>
            </div>
          ) : displays.length === 0 ? (
            /* Empty state */
                    ...
                    <tr className="bg-white/5 border-b border-white/8">
                      {columns.map((col) => (
                        <th
                          ...
                          className={`px-5 py-3 text-left text-xs uppercase tracking-wider font-semibold text-white/50 whitespace-nowrap select-none ${
                            col.sortable ? "cursor-pointer hover:text-white/80" : ""
                  ...
                  <tbody>
                    {displays.map((display) => (
                      ...
                      >
                        ...
                        {/* Name */}
                        <td className="px-5 py-3.5">
                          <span className="text-white text-sm font-medium">{display.displayName}</span>
                        </td>

                        {/* Location */}
                        <td className="px-5 py-3.5">
                          <span className="text-white/50 text-sm">{display.location}</span>
                        </td>
                        ...
                        <td className="px-5 py-3.5 text-right">
                          <div className="inline-flex items-center gap-1 justify-end">
                            <button
                              ...
                              disabled={!display.id}
                              className="inline-flex items-center gap-1.5 text-white/60 hover:text-white px-3 py-1.5 rounded-lg hover:bg-white/5 text-xs font-medium"
                              style={{ transition: "color 120ms ease, background-color 120ms ease" }}
                            >
                              <Pencil size={13} />
                              Edit
